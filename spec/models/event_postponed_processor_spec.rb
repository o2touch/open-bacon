...
describe EventPostponedProcessor do
  ...
  describe 'process' do
    it 'rejects message if the obj_type is not recognised' do
      ...
      EventPostponedProcessor.new('processor').process(mock_notification_item).should be_false
    end
    ...
    it 'sends a delayed event postponed email to the user', :sidekiq => false do
      ...
      event.invitees.each do |u|
        ...
        UserMailer.should_receive(:event_postponed).once.with(event.id, u.id, user.id)
      end

      EventPostponedProcessor.new('processor').process(mock_notification_item).should be_true
    end
  end
end
