...
class Campaign
  ...
  def self.all
    response = RestClient.get "https://api:#{MailgunResource.password}@api.mailgun.net/v2/mail.mitoo.co/campaigns"
    JSON.parse(response.body)
  end
  
  def self.send (from, to, subject, text, html, campaign_id, testmode=false)
    RestClient.post("https://api:#{MailgunResource.password}@api.mailgun.net/v2/mail.mitoo.co/messages",
                  :from => from,
                  :to => to,
                  :subject => subject,
                  :text => text,
